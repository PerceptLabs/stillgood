...
const performanceAreas = [
  ...
  {
    id: "multitasking",
    title: "Using several things",
    shortTitle: "multitasking",
    strength: () =>
      "Several active browser tasks remained responsive while work overlapped.",
    relativeLimit:
...
function buildPerformanceProfile(result, guide) {
  ...
  const middle = medianScore(scores);
  ...
  const strengths = ranked.filter(
    (area) => area.score >= Math.max(88, middle + 4),
  );
  for (const area of ranked) {
    if (strengths.length >= 3) break;
    if (
      !strengths.some((item) => item.id === area.id) &&
      area.score >= 84 &&
      highest - area.score <= 10
    ) {
      strengths.push(area);
    }
  }

  const limits = wellRounded
    ? []
    : [...areas]
        .filter((area) => area.score <= middle - 5 || area.score < 68)
  ...
  return {
    ...
    limits: limitItems,
    summary: wellRounded
      ? "Performance is well balanced across everyday tasks, with no clear weak area in this test."
      : limitItems.length
        ? `This device is strongest for ${joinPlainLanguage(strengthItems.map((item) => item.title.toLowerCase()))}. It reaches its limits sooner with ${joinPlainLanguage(limitItems.map((item) => areas.find((area) => area.id === item.id)?.shortTitle))}.`
        : `This device is strongest for ${joinPlainLanguage(strengthItems.map((item) => item.title.toLowerCase()))}, with no major weakness in ordinary work.`,
  };
}
