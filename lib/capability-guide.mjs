...
export function buildCapabilityGuide(result) {
  ...
  if ((result.graphics?.score ?? 0) < 68) {
    cautions.push({
      title: result.browserCompatibility?.isolatedGraphics
        ? "This browser struggles with visual effects"
        : "Animation-heavy pages",
      detail:
        result.browserCompatibility?.isolatedGraphics
          ? "Visual rendering was much slower than the computer's other work. Dense motion may stutter in this browser even though ordinary office and browsing tasks perform better."
