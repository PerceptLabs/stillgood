...
function buildPerformanceProfile(result, guide) {
  ...
  return {
    ...
    summary: wellRounded
      ...
      : limitItems.length
        ? `Best results came from ${joinPlainLanguage(strengthItems.map((item) => item.title.toLowerCase()))}. ${joinPlainLanguage(limitItems.map((item) => areas.find((area) => area.id === item.id)?.shortTitle))} had less reserve.`
        : `Results were strongest in ${joinPlainLanguage(strengthItems.map((item) => item.title.toLowerCase()))}, with no major weakness in ordinary work.`,
  };
}
...
export function buildCapabilityGuide(result) {
  ...
  guide.topSummary = guide.performanceProfile.wellRounded
    ...
    : limitNames.length
      ? `Comfortable for everyday work. ${joinPlainLanguage(limitNames)} ${limitNames.length === 1 ? "is" : "are"} likely to slow first.`
