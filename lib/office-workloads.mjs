import { createTextSortRanker } from "./benchmark-compatibility.mjs";
...
const senderSortRanker = createTextSortRanker(SENDERS);
const modelSortRanker = createTextSortRanker(MODELS);
...
export function buildEmailDataset(seed, size) {
  ...
  return Array.from({ length: size }, (_, index) => {
    ...
    return {
      id: index + 1,
      sender,
      senderSortRank: senderSortRanker.rankFor(sender),
      subject,
      body,
      kind,
      threadLength,
...
export function createEmailView(messages, actionIndex, domRows, seed) {
  ...
  matching.sort(
    action === 2
      ? (left, right) =>
          left.senderSortRank - right.senderSortRank ||
          left.id - right.id
...
export function buildWritingDataset(seed, targetWords) {
  ...
  const tableRows = Array.from({ length: 36 }, (_, index) => ({
    ...
    modelSortRank:
      modelSortRanker.rankFor(MODELS[(seed + index * 7) % MODELS.length]),
  ...
  if (action === 2) {
    ...
    tableRows = [...documentModel.tableRows].sort(
      (left, right) =>
        ...
        left.modelSortRank - right.modelSortRank ||
        left.id - right.id,
    );
  } else if (action === 6) {
