...
export function summarizeUpperReserve({
  ...
  if (mixedReserve?.tested) {
      ...
      if (
        ...
        const advancedStartupScore = Number.isFinite(level.advancedStartupMs)
          ? normalizeLower(level.advancedStartupMs, [
              { value: extended ? 650 : 450, score: 100 },
              { value: extended ? 1100 : 750, score: 94 },
              { value: extended ? 1800 : 1250, score: 85 },
              { value: extended ? 3200 : 2200, score: 72 },
              { value: extended ? 6000 : 4200, score: 52 },
    ...
    return {
      ...
      gradeCeiling,
      components,
      levels: scoredLevels.map((level) => ({ id: level.id, score: Math.round(level.score) })),
    };
  }
...
export function summarizeThoroughRun(metrics) {
  ...
  const storagePenalty =
    storage.available && storage.score < 90
      ? Math.min(6, (90 - storage.score) * 0.16)
      : 0;
