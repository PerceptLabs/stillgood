...
export function gradeForScore(score) {
  ...
  if (score >= 95) return { grade: "A", label: "Fast" };
  if (score >= 88) return { grade: "B+", label: "Strong second-life" };
  if (score >= 78) return { grade: "B", label: "Comfortable second-life" };
...
export function summarizeHeadroom(
  categories,
  multitasking,
  graphics,
  memory = null,
  ...
  const evidence = [
    { score: median(categoryEvidence), weight: 0.58 },
    ...
    { score: memory?.score, weight: 0.12, available: memory?.available },
    { score: storage?.score, weight: 0.05, available: storage?.available },
  ].filter(
    (item) =>
...
export function summarizeMemory(tiers = [], supported) {
  ...
  const summaries = validTiers.map((tier) => {
    ...
    const score = Math.round(foregroundScore + copyScore * 0.2);
    return {
      ...tier,
      copyOverheadMs,
      score,
...
export function summarizeStorage(
  ...
  const opfsScore = opfsSummaries.length
    ? Math.round(
        opfsSummaries.reduce((sum, tier) => sum + tier.score, 0) /
          opfsSummaries.length,
      )
    : 0;
  const sources = [
    {
...
export function summarizeThoroughRun(metrics) {
  ...
  if (memory.available)
    scoredCategories.push({ score: memory.score, weight: 0.06 });
  if (storage.available)
    scoredCategories.push({ score: storage.score, weight: 0.04 });
