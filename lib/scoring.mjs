...
function smoothOpportunityBonus(score, lower, upper, maximum) {
  if (!Number.isFinite(score) || score <= lower) return 0;
  if (score >= upper) return maximum;
  const progress = (score - lower) / (upper - lower);
  const eased = progress * progress * (3 - 2 * progress);
  return eased * maximum;
}

export function reserveOpportunityAward(upperReserve) {
  ...
  const extendedScore = Number.isFinite(extendedScore1000)
    ...
    : null;
  const standardBonus = smoothOpportunityBonus(standardScore, 75, 98, 4);
  // The final three points are deliberately difficult to earn. A device must
  // finish the distinct extended level and remain exceptional throughout it.
  const extendedBonus = smoothOpportunityBonus(extendedScore, 98, 100, 3);
  return {
    tested: true,
    standardScore1000,
    extendedScore1000,
    standardBonus1000: Math.round(standardBonus * 10),
    extendedBonus1000: Math.round(extendedBonus * 10),
    totalBonus1000: Math.round((standardBonus + extendedBonus) * 10),
  };
}
...
export function summarizeThoroughRun(metrics) {
  ...
  const reserveBaseCap = 93;
  const scoreAfterReserveBaseCap = Math.min(scoreBeforeReserve, reserveBaseCap);
  const reserveAward = reserveOpportunityAward(upperReserve);
  score = Math.min(
    100,
    scoreAfterReserveBaseCap + reserveAward.totalBonus1000 / 10,
  );

  const score1000 = toScore1000(score);
  ...
  const internalScoring = {
    ...
    safeguards: {
      ...
      memoryGradeCeiling: Number.isFinite(memory.gradeCeiling)
        ...
        : 1000,
      reserveBaseCap: reserveBaseCap * 10,
