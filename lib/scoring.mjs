...
export function classifyIsolatedBrowserGraphics({
  coreEverydayScores,
  graphics,
  video,
  longTaskSupported,
  ...
  const validCoreScores = coreEverydayScores.filter(Number.isFinite);
  const coreEverydayMedian = validCoreScores.length
    ? median(validCoreScores)
    : 0;
  const graphicsEverydayScore =
    graphics?.everydayScore ?? graphics?.score ?? 0;
  const telemetryLimited =
    longTaskSupported === false &&
    longAnimationFrameSupported === false;
  const visualGap = coreEverydayMedian - graphicsEverydayScore;
  const applied =
    telemetryLimited &&
    graphics?.available !== false &&
    graphicsEverydayScore < 58 &&
    coreEverydayMedian >= 80 &&
    video?.available !== false &&
    (video?.score ?? 0) >= 90 &&
    visualGap >= 30;

  return {
    isolatedGraphics: applied,
    telemetryLimited,
    coreEverydayMedian: Math.round(coreEverydayMedian),
    graphicsEverydayScore: Math.round(graphicsEverydayScore),
    visualGap: Math.round(visualGap),
    policy: "separate-isolated-browser-graphics",
  };
}

export function classifyTelemetryLimitedOpenCeiling({
  coreEverydayScores,
  headroom,
  telemetryLimited,
}) {
  const validCoreScores = coreEverydayScores.filter(Number.isFinite);
  const coreEverydayMinimum = validCoreScores.length
    ? Math.min(...validCoreScores)
    : 0;
  const applied =
    telemetryLimited === true &&
    coreEverydayMinimum >= 96 &&
    headroom?.extendedCategories >= 4 &&
    headroom?.openCeilings >= 4 &&
    headroom?.score >= 78 &&
    headroom?.score < 88;

  return {
    openCeilingHeadroom: applied,
    coreEverydayMinimum: Math.round(coreEverydayMinimum),
    openCeilings: headroom?.openCeilings ?? 0,
    proportionalHeadroomCap: applied
      ? Math.round(headroom.score + 7)
      : null,
  };
}
...
export function summarizeThoroughRun(metrics) {
  ...
  const graphicsCompatibility = classifyIsolatedBrowserGraphics({
    coreEverydayScores,
    graphics,
    video,
    longTaskSupported: metrics.longTaskSupported,
    longAnimationFrameSupported: metrics.longAnimationFrameSupported,
  });
  const graphicsSetsComputerGrade =
    graphics.available && !graphicsCompatibility.isolatedGraphics;
  const headroom = summarizeHeadroom(
    [browsing, email, writing, spreadsheets],
    multitasking,
    graphicsSetsComputerGrade
      ? graphics
      : { ...graphics, available: false },
  );
  const browserCompatibility = {
    ...
    ...classifyTelemetryLimitedOpenCeiling({
      coreEverydayScores,
      headroom,
      telemetryLimited: graphicsCompatibility.telemetryLimited,
    }),
  };
  ...
  if (graphicsSetsComputerGrade && graphics.everydayScore < 76)
    score = Math.min(score, 83);
  if (video.available && video.score < 76) score = Math.min(score, 83);
  if (graphicsSetsComputerGrade && graphics.everydayScore < 68)
    score = Math.min(score, 75);
  if (video.available && video.score < 68) score = Math.min(score, 75);
  if (graphicsSetsComputerGrade && graphics.everydayScore < 58)
  ...
  if (headroom.score < 88) {
    const proportionalHeadroomCap = Math.round(headroom.score + 7);
    score = browserCompatibility.openCeilingHeadroom
      ? Math.min(score, proportionalHeadroomCap)
      : Math.min(score, proportionalHeadroomCap, 87);
  }
  ...
  if (browserCompatibility.isolatedGraphics)
    integrityNotes.push(
      "This browser's isolated visual-rendering limitation is reported separately and did not set the computer grade.",
    );
  if (browserCompatibility.openCeilingHeadroom)
    integrityNotes.push(
      "This browser completed every adaptive reserve tier while standard congestion telemetry was unavailable. Reserve reduced the result proportionally instead of imposing the general hard grade ceiling.",
    );
  const roles = [];
