import { normalizeBrowserEvidence } from "./browser-normalization.mjs";
...
export function summarizeThoroughRun(metrics) {
  ...
  const rawHeadroom = summarizeHeadroom(
    [browsing, email, writing, spreadsheets],
    multitasking,
    graphics,
  );
  const browserNormalization = normalizeBrowserEvidence({
    browserFamily: metrics.browserFamily,
    graphics,
    headroom: rawHeadroom,
  });
  const gradeGraphics = {
    ...graphics,
    score: browserNormalization.normalizedGraphicsScore,
    everydayScore:
      browserNormalization.normalizedGraphicsEverydayScore,
  };
  const headroom = {
    ...rawHeadroom,
    score: browserNormalization.normalizedHeadroomScore,
    label: headroomLabelForScore(
      browserNormalization.normalizedHeadroomScore,
    ),
  };
  ...
  if (gradeGraphics.available)
    scoredCategories.push({ score: gradeGraphics.score, weight: 0.13 });
  if (video.available)
  ...
  if (gradeGraphics.available && gradeGraphics.everydayScore < 76)
    score = Math.min(score, 83);
  if (video.available && video.score < 76) score = Math.min(score, 83);
  if (gradeGraphics.available && gradeGraphics.everydayScore < 68)
    score = Math.min(score, 75);
  if (video.available && video.score < 68) score = Math.min(score, 75);
  if (gradeGraphics.available && gradeGraphics.everydayScore < 58)
  ...
  if (browserNormalization.applied)
    integrityNotes.push(
      `Firefox reference calibration ${browserNormalization.profileVersion} was applied to graphics and reserve evidence for the computer grade. Raw browser measurements remain in the exported result.`,
    );
  const roles = [];
