...
export function summarizeLatencyTiers(tiers, thresholds = {}) {
  ...
  const rawScore =
    summaries.reduce(
      (sum, summary, index) =>
        sum + statusValue(summary.status) * weights[index],
      0,
    ) / weightTotal;
  const medianCv = median(summaries.map((summary) => summary.cv));
  const stabilityPenalty =
    medianCv > 0.5 ? 12 : medianCv > 0.35 ? 7 : medianCv > 0.22 ? 3 : 0;

  return {
    score: Math.max(0, Math.round(rawScore - stabilityPenalty)),
    tiers: summaries,
    medianCv,
    highestComfortable:
...
export function summarizeThoroughRun(metrics) {
  const everyday = summarizeLatencyTiers(metrics.everydayTiers);
  const documents = summarizeLatencyTiers(metrics.documentTiers, {
    comfortableMedian: 240,
    comfortableWorst: 600,
    usableMedian: 600,
    usableWorst: 1500,
  });
  ...
  const scoredCategories = [
    { score: everyday.score, weight: 0.26 },
    { score: documents.score, weight: 0.19 },
    { score: multitasking.score, weight: 0.25 },
    { score: storage.score, weight: 0.03 },
    { score: recoveryScore, weight: 0.04 },
  ];
  if (graphics.available)
    scoredCategories.push({ score: graphics.score, weight: 0.11 });
  if (video.available)
  ...
  if (graphics.available && graphics.score < 76) score = Math.min(score, 91);
  if (video.available && video.score < 76) score = Math.min(score, 91);
  if (graphics.available && graphics.score < 58) score = Math.min(score, 83);
  if (video.available && video.score < 58) score = Math.min(score, 83);
  ...
  const roles = [];
  if (everyday.score >= 58) roles.push("Web and email");
  if (documents.score >= 58) roles.push("Documents and PDFs");
  if (multitasking.score >= 68) roles.push("Light multitasking");
  if (multitasking.score >= 84) roles.push("Heavy web multitasking");
  if (video.highestUsable !== "None")
    roles.push(`Video up to ${video.highestUsable}`);
