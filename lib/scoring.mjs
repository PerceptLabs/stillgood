...
export function summarizeLatencyTiers(tiers, thresholds = {}) {
  ...
  const capacityScore = capacityWeightTotal
    ? capacitySummaries.reduce(
        (sum, summary, index) =>
          sum + summary.score * capacityWeights[index],
        0,
      ) / capacityWeightTotal
    : everydayScore;
...
export function summarizeUpperReserve({
  ...
  if (mixedReserve?.tested) {
    const loadedP95Ms = Number.isFinite(mixedReserve.loadedP95Ms)
      ? mixedReserve.loadedP95Ms
      : 3000;
    const loadedWorstMs = Number.isFinite(mixedReserve.loadedWorstMs)
      ? mixedReserve.loadedWorstMs
      : 6000;
    const slowdownRatio = Number.isFinite(mixedReserve.slowdownRatio)
      ? Math.max(1, mixedReserve.slowdownRatio)
      : 8;
    ...
    const frameScore = Number.isFinite(mixedReserve.onTimeRatio)
      ? normalizeHigher(mixedReserve.onTimeRatio, [
    ...
    const score = Math.round(
      weightedGeometricMean([
        { score: responseScore, weight: 0.4 },
        { score: slowdownScore, weight: 0.32 },
        { score: tailScore, weight: 0.18 },
    ...
    return {
      ...
      gradeCeiling,
      components: [
        { id: "mixed-response", score: Math.round(responseScore), weight: 0.4 },
        { id: "slowdown", score: Math.round(slowdownScore), weight: 0.32 },
        { id: "tail", score: Math.round(tailScore), weight: 0.18 },
        { id: "frame-delivery", score: Math.round(frameScore), weight: 0.1 },
      ],
    };
  }
  ...
  const score = Math.round(weightedGeometricMean(components));
  const gradeCeiling =
    score >= 94
      ? 100
      : score >= 88
      ...
      : score >= 86
        ...
        : score >= 76
          ...
          : score >= 64
            ? "Moderate reserve"
            : "Everyday work is stronger than sustained reserve";
  return { tested: true, score, label, gradeCeiling, components };
}
...
export function summarizeThoroughRun(metrics) {
  ...
  const scoredCategories = [
    { score: browsing.score, weight: 0.25 },
    { score: email.score, weight: 0.1 },
    { score: writing.score, weight: 0.12 },
    { score: spreadsheets.score, weight: 0.12 },
    { score: multitasking.score, weight: 0.19 },
    { score: recoveryScore, weight: 0.04 },
  ];
  if (graphics.available)
    scoredCategories.push({ score: graphics.score, weight: 0.08 });
  if (video.available)
    scoredCategories.push({ score: video.score, weight: 0.04 });
  if (responsiveness.available)
    scoredCategories.push({ score: responsiveness.score, weight: 0.1 });
  ...
  if (graphics.available)
    webExperienceCategories.push({ score: graphics.score, weight: 0.08 });
  if (video.available)
    webExperienceCategories.push({ score: video.score, weight: 0.04 });
  if (responsiveness.available)
  ...
  const ceilingReached =
    ...
    invalidMeasurementCount === 0 &&
    variability <= 0.12 &&
    (!upperReserve.tested || upperReserve.score >= 94);
  ...
  score = Math.min(score, continuousHeadroomCeiling(headroom.score));
  ...
  if (upperReserve.tested)
    score = Math.min(score, upperReserve.gradeCeiling);
  if (metrics.reserveEvaluationComplete === true && !upperReserve.tested)
