...
A broadly strong preliminary result must also pass a paired mixed-workload
reserve stage before it can claim an A-range result. The same deterministic
browsing, inbox, document, spreadsheet, and Canvas actions run immediately
before and during controlled video, worker, memory, and persistent-save
pressure. Only exceptionally responsive devices continue to a distinctly
higher pressure level.

Version 6.19 makes ordinary responsiveness 78% of each core application score
...
the browser-evidence policy, `docs/36_BENCHMARK_V6_19_PAIRED_RESERVE_REPEATABILITY.md`
