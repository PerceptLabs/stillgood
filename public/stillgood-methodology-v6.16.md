...
**Benchmark profile:** `6.16.0-media-confirmation`  
**Published:** July 2026
...
- bundled local H.264 video playback from 480p through an adaptive 4K headroom tier;
...
- video completion, delivered and dropped frames, sustained stalls, and optional decoder-capability hints;
...
Version 6.16 applies no browser-name multiplier, offset, cap exception, or
...
When a lower resolution performs worse than a higher one, the lower tier is repeated twice and the median of all three attempts becomes the recorded result. This conditional confirmation distinguishes a one-time decoder-startup interruption from a repeatable playback limitation without lengthening normal runs.

The everyday video score uses only 480p30, 720p30, and 1080p30. After strong web, office, visual, and 1080p results, StillGood may fetch and run 1080p60 and 1440p30. It attempts 4K30 only when both preceding headroom tiers are comfortable. A browser's Media Capabilities response can prevent an unsupported tier from running, but the actual playback observation remains authoritative. Skipped or unsuccessful extended tiers identify the practical media ceiling without lowering the everyday video score or confidence.
...
mechanismsÃ¢â‚¬â€text, layout, arrays, formulas, memory, and renderingÃ¢â‚¬â€but LibreOffice
...
- [Media Capabilities decoding information](https://developer.mozilla.org/en-US/docs/Web/API/MediaCapabilities/decodingInfo)
