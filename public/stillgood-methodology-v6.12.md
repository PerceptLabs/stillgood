...
Version 6.11 corrects a reserve-scoring discontinuity found by paired
workstation testing. The general rule placed a hard B+ ceiling on any headroom
score below 88, even when every ordinary workload was excellent and every
maximum adaptive tier completed without finding the device's limit. In a
browser without both detailed congestion collectors, that hard ceiling is
replaced by the existing proportional `headroom + 7` cap only when the lowest
everyday score is at least 96 and all four adaptive categories leave the test
ceiling open. Reserve still lowers the result, but a one-point boundary cannot
create an eight-point final-score cliff. The full-telemetry path is unchanged.

Version 6.12 replaces the two conditional scoring exceptions with an explicit
Firefox reference calibration. Chromium is the frozen reference and uses
identity factors. Firefox graphics and aggregate headroom are multiplied by
versioned factors derived from matched HP and Dell runs before the computer
grade is calculated. Raw Firefox scores, tier timings, factors, profile
...
- [WebXPRT results calculation](https://www.principledtechnologies.com/benchmarkxprt/counter.php?inline=true&redirect=%2Fbenchmarkxprt%2Fwhitepapers%2Fwebxprt%2FWebXPRT-4-results-calculation.pdf)
