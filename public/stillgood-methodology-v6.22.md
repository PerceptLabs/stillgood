# StillGood methodology v6.22

**Benchmark profile:** `6.22.0-reserve-opportunity-bonus`
**Result schema:** `stillgood-result.v6.22`
...
Ordinary evidence and practical safeguards establish up to 93 points. Standard
reserve can add up to four points through a smooth opportunity curve beginning
only when reserve behavior is strong. Only devices that remain exceptionally
responsive at the distinct extended level can earn the final three points.
Reserve never changes an ordinary category and never subtracts from the score
already established before the stage. An unsupported, interrupted, or weak
attempt receives no bonus and a plain-language reserve limitation.
...
- The pre-reserve result, the 93-point opportunity cap, and the separate
  standard and extended reserve awards remain visible in the exported internal
  matrix. Only the final seven points depend on reserve performance.
